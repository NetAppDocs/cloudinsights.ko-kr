...
* CM(Telegraf ConfigMaps)을 편집하고 이전 API 토큰의 모든 인스턴스를 새 API 토큰으로 대체합니다.
+
....
...
:wq!
....
* DS(Telegraf DemonSet) 및 RS(ReplicaSet)를 편집하고 이전 API 토큰의 모든 인스턴스를 새 API 토큰으로 교체합니다.
+
....
...
:wq!
....
* Kubernetes 컨트롤러가 자동으로 Telegraf DS Pod를 다시 시작합니다.
* Telegraf RS 포드를 다시 시작합니다
+
