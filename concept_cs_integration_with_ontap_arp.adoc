...
| 공격이 감지된 후 24시간 이내에 이메일 경고가 수신됩니다. UI에서 알림은 Cloud Insights 워크로드 보안에서 이메일을 수신하기 24시간 전에 표시됩니다. | ONTAP가 _랜섬웨어 탐지_이벤트를 Cloud Insights 워크로드 보안(예: Cloud Secure)으로 전송하면 이메일이 전송됩니다. 이벤트에는 공격 목록과 타임 스탬프가 포함됩니다. 워크로드 보안 UI는 첫 번째 공격 파일의 경고 타임스탬프를 표시합니다. ONTAP는 특정 수의 파일이 인코딩될 때 _ 랜섬웨어 탐지 _ 이벤트를 Cloud Insights로 전송합니다. 따라서 알림이 UI에 표시되는 시간과 이메일을 보낸 시간 사이에 차이가 있을 수 있습니다. 
|===