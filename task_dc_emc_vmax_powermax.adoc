...
* 성능에 대한 액세스 검증: 획득 장치의 웹 브라우저에서 로 이동합니다 https://<SMI-S[] 호스트 이름 또는 IP>:5989/전자 구성. 여기서 "SMI-S 호스트 이름 또는 IP"는 SMI-S 서버의 IP 주소 또는 호스트 이름입니다. 이 URL은 EMC SMI-S("ECOM") 서비스의 관리 포털용 URL로, 로그인 팝업을 받게 됩니다.
