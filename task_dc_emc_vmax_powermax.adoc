...
| "알 수 없는 코드" 메시지와 함께 데이터 수집 오류가 표시됩니다. | Solutions Enabler 서버의 데몬 구성 파일에 사용 권한이 선언되지 않은 경우 이 메시지가 표시될 수 있습니다(참조) <<요구 사항,요구 사항>> 위) SE 클라이언트 버전이 SE 서버 버전과 일치하는 것으로 가정합니다. 
|===
