...
IMPORTANT: 이 페이지는 현재 진행 중입니다. Kubernetes에 에이전트를 설치하는 방법에 대한 최신 지침은 link:task_config_telegraf_agent.html["* 여기로 이동 *"].
...
NOTE: Agent를 설치하기 전에 설치 파일을 확인하려면 정보 를 참조하십시오 <<Verifying Checksums>>.
...
Kubernetes에 에이전트를 설치하려면 먼저 다음을 수행해야 합니다.
...
.전제 조건:
* NetApp Kubernetes Monitoring Operator 설치가 테스트되었으며 다음 구성 요소에 대해 작동할 것으로 예상됩니다.
+
** AWS EKS 1.18-1.21
** OpenShift 4.6-4.8
** Rancher 2.4 - 2.6.
