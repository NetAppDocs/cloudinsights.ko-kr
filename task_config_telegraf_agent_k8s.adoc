...
operator-config.yAML에서 모니터링-운영자 배포를 편집하고 모든 이미지 참조를 수정하여 개인 Docker 저장소를 사용하십시오.
...
| 운용자 설치 후, Telegraf-ds Pod는 CrashLoopBackOff로 진입하고 POD 로그는 "su:Authentication failure"를 나타낸다. | NetApp-monitoring-configuration agentconfiguration을 편집하고 set_dockerMetricCollectionEnabled_to false를 설정합니다.  자세한 내용은 조작자를 참조하십시오 link:telegraf_agent_k8s_config_options.html["구성 옵션"]. 
|===
