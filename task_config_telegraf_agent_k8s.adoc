...
* _curl, sudo, openssl, sha256sum, _ 및 _kubbctl_ 명령을 사용할 수 있어야 합니다. 최상의 결과를 얻으려면 이러한 명령을 경로에 추가하십시오. 최소한 _ 에이전트, 클러스터 역할, clusterrobindings, customresourcedefinitions, Deployments 등의 Kubernetes 개체에 대한 액세스 권한으로 _kubbctl_을 구성해야 합니다. 네임스페이스, 역할, rolebindings, 비밀, serviceaccount, _ 및 _services _. 을 참조하십시오 link:nkmo_clusterrole_yaml.html["여기"] 예를 들어, 최소 클러스터 역할 권한이 있는 .YAML 파일을 예로 들 수 있습니다.
