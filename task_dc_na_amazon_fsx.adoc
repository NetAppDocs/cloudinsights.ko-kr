...
NOTE: 이 Cloud Insights Collector에는 _filesystem-scoped_role이 있는 ONTAP 사용자가 필요합니다. AWS를 검토하십시오 link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/roles-and-users.html["역할 및 규칙"] 사용 가능한 옵션에 대한 설명서. 현재 AWS는 Filesystem Scope인 _fsxadmin_의 한 가지 사용자 역할만 지원합니다. Cloud Insights Collector에 사용할 적절한 역할입니다.
...
FSx CLI 명령은 FSx가 API를 통해 금지하는 일부 용량 값을 검색합니다. Cloud Insights이 API를 통해 FSx 데이터를 검색하려고 하므로 Cloud Insights의 특정 용량 값(예: 스토리지 풀에 대한 값)이 FSx 자체에 있는 값과 다를 수 있습니다. 자세한 내용은 아마존 담당자에게 문의하십시오.
