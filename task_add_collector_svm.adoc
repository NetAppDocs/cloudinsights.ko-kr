...
에 대해 다음 사항을 염두에 두십시오 link:/cloudinsights/cs_restrict_user_access.html["사용자 액세스 차단"]:
...
....
참고: 하나의 REST-role이 이미 추가된 경우(_arwrole_or_csrestrole_-) 두 번째 REST-role을 추가할 필요가 없습니다. 아래 예와 같이 간단히 API 권한을 추가할 수 있습니다.

예: _csrestrole_이 이미 있으므로 랜섬웨어 방지 기능을 활성화하고 기존_csrestrole_ 에 API 권한을 부여하기만 하면 됩니다.
...
security login create -user-or-group-name csuser -application http -authmethod password -role arwrole
....
자세한 내용은 정보 를 참조하십시오 link:concept_cs_integration_with_ontap_arp.html["ONTAP Autonomous 랜섬웨어 Protection과 통합"]



=== ONTAP 액세스에 대한 권한이 거부되었습니다
