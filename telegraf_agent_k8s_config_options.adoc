...
[role="lead"]
NetApp Kubernetes Monitoring Operator 설치 및 구성을 사용자 지정할 수 있습니다.
...
|  | agentUUID입니다 | 모든 Cloud Insights 연산자 설치에서 고유하게 식별되는 문자열입니다. 이 설정은 일반적으로 Cloud Insights에서 자동으로 설정합니다. 
