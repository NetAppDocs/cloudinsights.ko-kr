...
Cloud Insights에는 Collector의 옵션으로 ONTAP 클러스터 스위치 데이터를 수집하는 기능이 있습니다 <<advanced-configuration,고급 구성>> 설정. Cloud Insights Collector에서 이 기능을 활성화하는 것 외에도 ONTAP 시스템 * 자체에서 제공할 수 있도록 구성해야 합니다 link:https://docs.netapp.com/us-en/ontap-cli-98/system-switch-ethernet-create.html["스위치 정보"]를 선택하고 가 올바른지 확인합니다 <<a-note-about-permissions,권한>> 스위치 데이터를 Cloud Insights로 전송할 수 있도록 설정됩니다.
...
| 클러스터 스위치 메트릭입니다 | Cloud Insights이 클러스터 스위치 데이터를 수집하도록 허용합니다. Cloud Insights 측에서 이 기능을 활성화하는 것 외에도 ONTAP 시스템에서도 이 기능을 제공하도록 구성해야 합니다 link:https://docs.netapp.com/us-en/ontap-cli-98/system-switch-ethernet-create.html["스위치 정보"]를 선택하고 가 올바른지 확인합니다 <<a-note-about-permissions,권한>> 스위치 데이터를 Cloud Insights로 전송할 수 있도록 설정됩니다. 아래의 "권한에 대한 참고 사항"을 참조하십시오. 
|===
...
NOTE: ONTAP 액세스 제어가 올바르게 설정되어 있지 않으면 Cloud Insights REST 호출이 실패하여 장치의 데이터 공백이 발생할 수 있습니다.  예를 들어, Cloud Insights Collector에서 활성화했지만 ONTAP에 대한 권한을 구성하지 않은 경우 획득이 실패합니다.  또한 역할이 이전에 ONTAP에 정의되어 있고 REST API 기능을 추가하는 경우 _http_가 역할에 추가되었는지 확인합니다.
