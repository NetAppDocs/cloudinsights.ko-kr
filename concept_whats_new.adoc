...
를 클릭합니다 link:/task_config_telegraf_agent_k8s.html#operator-based-install-or-script-based-install["NetApp Kubernetes 모니터링 운영자"] (NKMO)는 Kubernetes 1.17 이상을 실행하는 모든 환경에서 사용할 수 있으며 Cloud Insights용 Kubernetes를 설치하는 데 권장되는 방법입니다. NKMO는 Cloud Insights에서 제어가 가능한 보다 유연한 모니터링 구성과 고객 중심의 구성 개입을 줄이고 K8s 클러스터에서 실행 중인 다른 소프트웨어를 모니터링할 수 있는 기회를 강화함으로써 스크립트 설치 모니터링 방법에 비해 향상된 기능입니다.
...
이제 데이터 수집에 대한 알림을 받을 수 있습니다 link:/task_system_monitors.html#collector-monitors["이벤트"]:
...
이러한 모니터는 기본적으로 _일시 중지됨_이며 알림을 보내기 전에 활성화(재개)해야 합니다.

또한 Collector 경고용 모니터는 다른 모니터 유형보다 더 많은 경고를 보낼 수 있으므로 문제 해결 없이는 알림 수신자를 설정하지 않는 것이 좋습니다.



==== 저장소 변경 로그 이벤트에 대한 경고

Cloud Insights에서 생성한 스토리지 변경 로그 이벤트(로그 소스 _logs.cloud_insights.change_log.storage_ 사용)에 대한 모니터를 정의할 때 모든 수집 후 스토리지 시스템의 이전 상태와 새 상태를 비교할 때 정보 경고를 수신합니다. 기본 모니터는 다음 이벤트에 대해 ONTAP 저장소에 사용할 수 있습니다.
...
| Cloud Secure – 공격에 대한 사용자 액세스를 제한합니다 | 공격이 감지될 때 사용자 액세스를 제한하는 기능을 통해 업무상 중요한 데이터를 더욱 안전하게 보호할 수 있습니다. 자동 응답 정책을 사용하거나 알림 또는 사용자 세부 정보 페이지에서 수동으로 액세스를 제한할 수 있습니다.link:https://docs.netapp.com/us-en/cloudinsights/cs_automated_response_policies.html["자세히 보기"] 
...
* * Cohesity Smart Files * - 이 REST API 기반 수집기는 Cohesity 클러스터를 획득하여 "뷰"(CI 내부 볼륨)와 다양한 노드를 검색하고 성능 메트릭을 수집합니다.
