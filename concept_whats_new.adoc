...
새 워크로드 보안 API를 사용하여 데이터 수집기(추가/업데이트/삭제) 및 에이전트를 관리할 수 있습니다. Admin > API Access > API Documentation * 으로 이동한 후 _Workload Security_API 유형을 선택하여 자세히 알아보십시오.
...
워크로드 보안에는 다음과 같은 개선 사항이 포함됩니다.

* 위양성 경고의 수를 줄이기 위해 감지 알고리즘이 향상되었습니다.
* Forensics 페이지를 로드, 표시 및 스크롤하는 동안 사용자 인터페이스 성능이 향상되었습니다.
