...
Cloud Insights는 관찰 가능성 및 워크로드 보안 화면을 보다 효율적으로 표시하여 사용자 환경을 개선함으로써 이러한 기능을 탐색하는 동안 로드 시간을 줄여줍니다. 워크로드 보안 페이지 URL이 변경되었습니다. 이전 URL은 자동으로 새 URL로 리디렉션됩니다.
...
를 클릭합니다 link:concept_kubernetes_performance_monitoring_and_map.html["_Kubernetes 네트워크 성능 및 맵 _"] Kubernetes 워크로드 간의 종속성을 매핑하여 문제 해결을 간소화합니다. Kubernetes 네트워크 성능 지연 시간 및 이상 징후를 실시간으로 파악하여 사용자에게 영향을 미치기 전에 성능 문제를 식별할 수 있습니다. 이 기능은 조직이 Kubernetes 트래픽 흐름을 분석하고 감사하여 전체 비용을 절감할 수 있도록 도와줍니다.
