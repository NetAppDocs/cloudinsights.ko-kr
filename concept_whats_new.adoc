...
Cloud Insights에는 이제 객체 아이콘그래프가 업데이트되어 다양한 Cloud Insights 페이지에 사용되는 아이콘의 전체적인 분산을 줄이고 유사한 객체(예: 볼륨 관련 객체)를 그룹화합니다. 이전의 '문자 기반' 아이콘이 이미지 기반 그래픽으로 대체되어 Cloud Insights 화면을 보다 사용자 친화적이고 쉽게 사용할 수 있습니다.

아이콘의 의미를 잘 모르십니까? 아래 표(페이지에서도 사용 가능 link:reference_object_icons.html["개체 아이콘 참조"] )에 모두 설명되어 있습니다.
