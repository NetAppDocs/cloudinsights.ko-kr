...
=== Kubernetes 네트워크 맵에 대한 iSCSI SAN 지원

NFS 및 iSCSI에 대한 Kubernetes 기본 스토리지 종속성을 식별합니다.
Kubernetes 애플리케이션의 스토리지 네트워크 활동 및 정확한 스토리지 메트릭에 대한 통찰력을 확보하여 문제를 빠르게 해결합니다. Kubernetes 애플리케이션이 비용청구 및 쇼백 보고를 위해 스토리지 인프라를 사용하는 방법을 이해합니다.
...
Cloud Insights의 초기 평가판 외에 의 이점을 활용할 수도 있습니다 link:concept_subscribing_to_cloud_insights.html#module-evaluation["모듈 평가"]. 예를 들어, 이미 Infrastructure 관측성 에 가입하고 Kubernetes를 환경에 추가하고 있는 경우 Kubernetes 관측성 30일 시험판에 자동으로 등록됩니다. 평가판 기간이 끝날 때까지만 Kubernetes 관측성 관리형 유닛 사용에 대한 요금을 청구합니다.
...
데이터 액세스를 보장하는 것이 필수이므로 .csv 내보내기를 만들었습니다 image:csv_export_icon.png["CSV 내보내기 아이콘"] 쿼리 중인 데이터 유형(자산 또는 통합)에 관계없이 모든 메트릭 쿼리, 대시보드 테이블 위젯 및 개체 랜딩 페이지에 사용할 수 있습니다.
...
Cloud Insights 프리미엄 에디션을 사용하면 관리자가 * 를 활성화할 수 있습니다link:concept_user_roles.html["SSO(Single Sign-On)"]개별적으로 초대할 필요 없이 회사 도메인의 모든 사용자에 대해 Cloud Insights에 대한 * (SSO) 액세스. SSO를 사용하면 동일한 도메인 이메일 주소를 가진 모든 사용자가 회사 자격 증명을 사용하여 Cloud Insights에 로그인할 수 있습니다.
...
Admin > Subscription * 페이지 link:concept_subscribing_to_cloud_insights.html["비교 표"] Cloud Insights의 기본, 표준 및 프리미엄 버전에서 사용할 수 있는 기능 집합을 나열하도록 업데이트되었습니다. NetApp은 계속해서 클라우드 서비스를 개선하고 있으므로, 이 페이지를 자주 방문하여 진화하는 비즈니스 요구사항에 맞는 에디션을 찾아보십시오.
